...
class TrainingSample(BaseModel):
    ...
    feature_graph: Dict[str, Any] = Field(
        default_factory=dict, description="FeatureGraph IR - executable geometry graph"
    )
    feature_graph_version: str = Field(
        default="v1", description="FeatureGraph schema version (v1, v2, v3)"
    )
    ...
    execution_trace: Dict[str, Any] = Field(
        default_factory=dict, description="Detailed execution trace from compiler"
    )
    retry_count: int = Field(
        default=0, description="Number of LLM retries before this result"
    )
    ...
    geometry_metrics: Optional[GeometryMetrics] = Field(
        None, description="Computed geometry properties (volume, surface area, etc.)"
    )
    ...
    llm_raw_response: str = Field(
        default="", description="Raw LLM response before parsing (for debugging)"
    )
    json_parse_success: bool = Field(
        default=True, description="Whether JSON parsing succeeded without repair"
    )
    json_repair_applied: bool = Field(
        default=False, description="Whether auto-repair was needed for JSON"
    )
    ...
    # =========================================================================
    backend: str = Field(
        default="build123d", description="CAD backend used (build123d, onshape)"
    )
    is_synthetic: bool = Field(
        default=False, description="Whether this is synthetic training data"
    )
    quality_score: float = Field(
        default=0.0, description="Computed quality score 0.0-1.0"
    )
    ...
    def to_training_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metadata": {
                ...
                "geometry_metrics": (
                    self.geometry_metrics.model_dump()
