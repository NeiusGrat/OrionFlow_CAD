...
class ManufacturingConstraints(BaseModel):
    ...
    min_tool_diameter: Optional[float] = Field(
        None, description="Minimum tool diameter in mm (CNC)"
    )
    max_tool_length: Optional[float] = Field(
        None, description="Maximum tool reach in mm (CNC)"
    )

    # Geometry constraints
    min_fillet_radius: Optional[float] = Field(
        None, description="Minimum fillet radius in mm (computed from tool)"
    )
    min_wall_thickness: Optional[float] = Field(
        None, description="Minimum wall thickness in mm"
    )
    ...
    min_hole_diameter: Optional[float] = Field(
        None, description="Minimum drillable hole diameter in mm"
    )

    # Feature constraints
    max_aspect_ratio: Optional[float] = Field(
        None, description="Max depth:width ratio for pockets/slots"
    )
    draft_angle_required: Optional[float] = Field(
        None, description="Draft angle in degrees (casting/injection molding)"
    )
    support_overhang_angle: Optional[float] = Field(
        None, description="Max overhang angle without supports (3D printing)"
    )
    ...
    typical_materials: List[str] = Field(
        default_factory=list, description="Common materials for this process"
    )
    ...
    @classmethod
    def for_sheet_metal(
        cls, material_thickness: float = 1.5
    ) -> "ManufacturingConstraints":
        """
...
class MaterialSpec(BaseModel):
    ...
    tensile_strength: Optional[float] = Field(
        None, description="Tensile strength in MPa"
    )
    ...
    def aluminum_6061(cls) -> "MaterialSpec":
        return cls(
            name="Aluminum 6061-T6", density=2.7, tensile_strength=310, cost_per_kg=5.0
        )
    ...
    def steel_1018(cls) -> "MaterialSpec":
        return cls(
            name="Steel 1018", density=7.87, tensile_strength=440, cost_per_kg=2.0
        )
