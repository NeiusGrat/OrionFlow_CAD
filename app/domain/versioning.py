...
class FeatureGraphDiff(BaseModel):
    ...
    added_features: List[str] = Field(
        default_factory=list, description="IDs of features added"
    )
    removed_features: List[str] = Field(
        default_factory=list, description="IDs of features removed"
    )
    modified_features: Dict[str, Dict[str, Tuple[Any, Any]]] = Field(
        default_factory=dict, description="feature_id → {param: (old_val, new_val)}"
    )

    # Parameter changes
    modified_parameters: Dict[str, Tuple[Any, Any]] = Field(
        default_factory=dict, description="param_name → (old_val, new_val)"
    )
    ...
    def apply(self, base_graph: FeatureGraphV3) -> FeatureGraphV3:
        ...
        for feature_id in self.removed_features:
            new_graph.features = [f for f in new_graph.features if f.id != feature_id]
...
class FeatureGraphVersion(BaseModel):
    ...
    tags: List[str] = Field(
        default_factory=list, description="Version tags: v1.0, production, etc."
    )
...
class VersionManager:
    ...
    def create_branch(
        self, branch_name: str, from_version_id: Optional[str] = None
    ) -> str:
        """
    ...
    def diff(self, version_a_id: str, version_b_id: str) -> FeatureGraphDiff:
        """
    ...
    def _compute_diff(
        self, graph_a: FeatureGraphV3, graph_b: FeatureGraphV3
    ) -> FeatureGraphDiff:
        ...
        # Added features
        diff.added_features = [fid for fid in features_b if fid not in features_a]

        # Removed features
        diff.removed_features = [fid for fid in features_a if fid not in features_b]
