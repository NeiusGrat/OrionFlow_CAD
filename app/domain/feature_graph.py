...
class SketchConstraint(BaseModel):
    ...
    entities: List[str] = Field(
        ..., description="IDs of entities affected by this constraint"
    )
    value: Optional[Union[str, float]] = Field(
        None, description="Constraint value (if applicable, e.g. distance)"
    )
...
class Feature(BaseModel):
    ...
    type: Literal["extrude", "cut", "fillet", "chamfer", "pattern", "revolve", "loft"]
    sketch: Optional[str] = Field(
        None, description="ID of the sketch to operate on (if applicable)"
    )
    params: Dict[str, Union[str, float]] = Field(
        ..., description="Operation parameters (e.g. {'depth': '$thickness'})"
    )
    depends_on: List[str] = Field(
        default_factory=list, description="IDs of parent features (for history)"
    )
...
class FeatureGraph(BaseModel):
    ...
    sketches: List[Sketch] = Field(
        default_factory=list, description="List of 2D sketches used by features"
    )

    # 5️⃣ Features (3D History)
    features: List[Feature] = Field(..., description="Ordered list of 3D operations")
    ...
    def validate_parameters(cls, v):
        ...
        for name, val in v.items():
            if not isinstance(val, (int, float)):
                raise ValueError(
                    f"Parameter '{name}' must be a number, got {type(val)}"
                )
        return v
    ...
    def strip_forbidden_fields(self) -> "FeatureGraph":
        ...
        clean_metadata = {
            k: v for k, v in (self.metadata or {}).items() if k not in forbidden
        }
