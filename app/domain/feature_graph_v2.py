...
class SemanticSelector(BaseModel):
    ...
    entity_ids: Optional[List[str]] = Field(
        None, description="Explicit entity UUIDs to target (Tier 1: highest priority)"
    )
    created_by_feature: Optional[str] = Field(
        None, description="Select entities created by this feature ID (Tier 2)"
    )
    semantic_roles: Optional[List[str]] = Field(
        None, description="Select entities with these semantic role tags (Tier 3)"
    ...
    def is_simple_string(self) -> bool:
        ...
        return (
            self.selector_type == SelectorType.STRING
            and self.string_selector is not None
        )
...
def semantic_selector(
    *filters: GeometricFilter, description: str = None
) -> SemanticSelector:
...
def parallel_to_axis(axis: str) -> GeometricFilter:
    ...
    return GeometricFilter(
        type=GeometricFilterType.PARALLEL_TO_AXIS, parameters={"axis": axis}
    )
...
def on_face(face_selector: str) -> GeometricFilter:
    ...
    return GeometricFilter(
        type=GeometricFilterType.ON_FACE, parameters={"face_selector": face_selector}
    )
...
def length_range(min_length: float = None, max_length: float = None) -> GeometricFilter:
    ...
    return GeometricFilter(type=GeometricFilterType.LENGTH_RANGE, parameters=params)
