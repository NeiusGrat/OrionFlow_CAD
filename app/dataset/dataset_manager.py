...
class DatasetManager:
    ...
    def create_dataset_version(
            ...
            json.dump(
                {
                    ...
                    "avg_complexity": sum(s.complexity_score for s in samples)
                    / max(len(samples), 1),
        ...
        logger.info(
            f"Created dataset version {version_name} with {len(samples)} samples"
        )
        return jsonl_path
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "with_validation_issues": sum(
                1 for s in samples if s.has_validation_issues
            ),
