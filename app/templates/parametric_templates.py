...
class BoxTemplate(ParametricTemplate):
    ...
    def generate(self, intent: DesignIntent) -> FeatureGraphV3:
        ...
        fillet_radius = intent.key_dimensions.get(
            "fillet_radius", min(width, depth, height) * 0.05
        )
        ...
        graph = FeatureGraphV3(
            version="3.0",
            units="mm",
            metadata={"template": self.name, "intent": intent.model_dump()},
            parameters={
                "width": width,
                "depth": depth,
                "height": height,
...
class BracketTemplate(ParametricTemplate):
    ...
    def generate(self, intent: DesignIntent) -> FeatureGraphV3:
        ...
        fillet_radius = intent.key_dimensions.get(
            "fillet_radius", min(thickness * 0.3, 3.0)
        )
