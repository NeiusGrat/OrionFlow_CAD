...
class DesignContext:
    ...
    feature_history: List[Dict] = field(
    ...
    last_topology_refs: Dict[str, List] = field(
...
class ContextEngine:
    ...
    def get_or_create_context(
        self, session_id: str, design_id: Optional[str] = None
    ) -> DesignContext:
        """Get existing context or create new one."""
        ctx = self.get_context(session_id)
        if ctx is None:
            ctx = self.create_context(
                design_id=design_id or session_id, session_id=session_id
            )
        return ctx
        ...
        if new_features:
            ...
            for feature_id in new_features:
                ctx.feature_history.append(
                    {"feature_id": feature_id, "timestamp": datetime.now().isoformat()}
                )
    ...
    def set_topology_refs(
        ...
        ctx = self.get_context(session_id)
        if ctx:
            ctx.last_topology_refs = {"edges": edges or [], "faces": faces or []}
    ...
    def _find_last_feature_of_type(
        self, ctx: DesignContext, feature_type: str
    ) -> Optional[str]:
