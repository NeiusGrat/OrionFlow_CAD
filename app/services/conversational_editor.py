...
class ConversationalEditor:
    ...
    async def apply_edit(
        self, current_graph: FeatureGraph, edit_request: str
    ) -> FeatureGraph:
        """
    ...
    def _try_heuristic_edit(
        self, graph: FeatureGraph, request: str
    ...
    async def _llm_based_edit(
        ...
        prompt = EDIT_PROMPT_TEMPLATE.format(
            current_graph=current_json, edit_request=edit_request
        )
        ...
        try:
            ...
            if hasattr(self.llm_client, "client") and isinstance(
                self.llm_client.client, AsyncGroq
            ):
                    ...
                    messages=[
                        {
                            "role": "user",
                            "content": prompt,
                        }  # Prompt contains system instructions effectively
                    ],
                    temperature=0.1,
                    max_tokens=4096,
