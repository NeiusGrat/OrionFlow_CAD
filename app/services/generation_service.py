...
class GenerationService:
    ...
    def decompose_prompt(prompt: str) -> DecomposedIntent:
        ...
        if (
            "hole" in keywords
            or "circle" in keywords
            or "cylinder" in keywords
            or "shaft" in keywords
        ):
        ...
        if (
            ...
            or "length" in keywords
            or "width" in keywords
        ):
    ...
    def generate_construction_plan(
        prompt: str, intent: DecomposedIntent
    ) -> ConstructionPlan:
        """
        ...
        if "base_profile" in intent.sketch_intent:
            ...
            if "box" in keywords or "rectangle" in keywords:
                ...
                parameters["width"] = PlanParameter(
                    unit="mm", default=50.0, min_value=1.0, semantic_name="Plate Width"
                )
        ...
        if "extrude" in intent.feature_intent:
            ...
            parameters["depth"] = PlanParameter(
                unit="mm", default=20.0, min_value=0.1, semantic_name="Extrusion Depth"
            )
        ...
        if len(numbers) >= 1 and "width" in parameters:
            parameters["width"] = parameters["width"].model_copy(
                update={
        ...
        if len(numbers) >= 2 and "height" in parameters:
            parameters["height"] = parameters["height"].model_copy(
                update={
        ...
        if len(numbers) >= 3 and "depth" in parameters:
            parameters["depth"] = parameters["depth"].model_copy(
                update={
    ...
    async def sync_to_onshape(
        self, feature_graph: FeatureGraph, doc_id: str, work_id: str, ele_id: str
    ) -> bool:
        """
        ...
        try:
            ...
            return self.onshape_client.update_featurescript(
                did=doc_id, wid=work_id, eid=ele_id, script_content=script_content
            )
    ...
    async def generate(
        ...
        onshape_context: Dict[str, str] = None,
    ) -> GenerationResult:
        """
        ...
        logger.info(
            f"Construction Plan: {len(construction_plan.construction_sequence)} steps, {len(construction_plan.parameters)} params"
        )
            ...
            try:
                ...
                if backend == "onshape":
                    if not onshape_context:
                        raise ValueError(
                            "Onshape backend requires onshape_context (did, wid, eid)"
                        )
                    ...
                    logger.warning(
                        "CadQuery backend deprecated, falling back to Build123d"
                    )
                ...
                if trace.success and solid:
                    ...
                    try:
                        ...
                        logger.info(
                            f"Training sample logged: {training_sample.sample_id}"
                        )
                    except Exception as e:
                ...
                if not trace.retryable or attempt == MAX_RETRIES:
                    ...
                    try:
                        ...
                        write_training_sample(training_sample)
                        logger.info(
                            f"Training sample (failure) logged: {training_sample.sample_id}"
                        )
                    except Exception as e:
                        logger.warning(f"Failed to log training sample: {e}")
                    # -----------------------
    ...
    async def regenerate(
        self, feature_graph_dict: dict, prompt: str = ""
    ) -> GenerationResult:
        """
