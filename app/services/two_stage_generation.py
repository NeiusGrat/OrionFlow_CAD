...
async def generate_two_stage(self, user_prompt: str) -> dict:
    """
    ...
    """
    logger.info("=== TWO-STAGE GENERATION START ===")
    ...
    try:
        ...
        logger.info(
            f"Intent extracted: part_type={intent.part_type}, template={intent.template_name()}"
        )
        ...
        step_path, stl_path, glb_path, trace = self.compiler.compile(
            feature_graph, f"two_stage_{hash(user_prompt)}"
        )
