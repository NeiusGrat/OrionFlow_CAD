...
def calculator_tools() -> list[dict]:
    ...
    for name, fn in sorted(calc.CALCULATORS.items()):
        ...
        for pname, param in sig.parameters.items():
            ...
            if container in (dict, list) or pname in ("payload", "parts",
                                                      "thinking", "variables"):
                structured = True
                break
...
def _run_calculator(name: str, arguments: dict) -> str:
    ...
    return json.dumps({k: (round(v, 6) if isinstance(v, float) else v)
                       for k, v in result.items()}, default=str)


# --------------------------------------------------------------------------- #
def _brief(spec: EngineeringSpecification, message: str,
           questions: list[str]) -> str:
    """Everything the planner needs and nothing it does not."""
    from orion.family_schema import check_guards, describe

    lines = [f"The user asked: {message}", "",
    ...
    if guards:
        ...
        lines += [f"  {g['id']}: {g['expr']} = {g['value']:.3f}"
                  + ("   <-- VIOLATED, you must fix this" if not g["holds"] else "")
                  for g in guards]
        if broken:
            lines.append("")
            lines.append("The baseline does not currently build. The dimensions "
                         "the user gave are fixed; change a default instead.")
    if spec.material or spec.process:
        lines.append("")
        lines.append(f"Material: {spec.material or 'unstated'}; "
...
def apply_overrides(spec: EngineeringSpecification, overrides: list[dict]
    ...
    for row in overrides:
        ...
        if known and name not in known:
            refused.append({**row, "reason":
                            f"{name!r} is not a {spec.part_class} variable; "
        ...
        broken = [g for g in check_guards(spec.part_class, trial)
                  if not g["holds"]]
        if broken:
            first = broken[0]
            refused.append({**row, "reason":
                            f"would violate {first['id']}: {first['expr']} "
    ...
    updated = EngineeringSpecification(
        part_class=spec.part_class, variables=variables,
        function=spec.function, manufacturing=spec.manufacturing,
        rationale=rationale, citations=list(spec.citations),
        calculations=dict(spec.calculations),
        constraints=list(spec.constraints),
        material=spec.material, process=spec.process)
...
class EngineeringPlanner:
    ...
    def __init__(self, complete: Optional[Callable] = None,
                 tool_rounds: int = PLAN_TOOL_ROUNDS):
        self._complete = complete
        self._rounds = tool_rounds

    # ------------------------------------------------------------------ #
    def baseline(self, message: str, intent=None) -> tuple[
            Optional[EngineeringSpecification], list[str]]:
    ...
    def plan(self, message: str, intent=None) -> PlanResult:
        ...
        if spec is None:
            return PlanResult(questions=questions,
                              error="could not identify the part family")

        result = PlanResult(specification=spec,
...
def live_completion(model: Optional[str] = None, provider: str = "vllm",
                    retries: int = 3):
    ...
    chosen = model or os.environ.get("ORION_CONVERSATION_MODEL",
                                     "orionflow-base")

    def to_message(row: dict) -> LLMMessage:
        role = row.get("role")
        if role == "tool":
            return LLMMessage.tool(row.get("content", ""),
                                   row.get("tool_call_id", ""),
                                   row.get("name", ""))
        if role == "assistant":
            return LLMMessage.assistant(row.get("content", ""),
    ...
    def complete(messages: list[dict], tools: Optional[list[dict]]):
        ...
        for attempt in range(retries):
            try:
                response = client.chat(wire, tools=tools or None,
            ...
            if response.finish_reason == "error" or \
                    str(response.content).startswith("[vllm transport error"):
