...
class Build123dCompiler:
    ...
    def compile(self, cfg: FeatureGraph, job_id: str) -> Tuple[Path, Path, Path]:
        ...
        try:
            ...
            return {"step": step_path, "stl": stl_path, "glb": glb_path}
    ...
    def _compile_sketch(self, ctx: BuildContext, sketch_def: Sketch):
        ...
        plane = (
            Plane.XY
            if sketch_def.plane == "XY"
            else Plane.YZ if sketch_def.plane == "YZ" else Plane.XZ
        )
    ...
    def _compile_feature(self, ctx: BuildContext, feature: Feature):
        ...
        def val(k):
            return self._resolve_param(ctx, feature.params.get(k))

        if feature.type == "extrude":
            if not feature.sketch:
                raise CompilationError(
                    f"Extrude feature '{feature.id}' missing sketch reference"
                )
            ...
            if not ctx.part:
                raise CompilationError("Cannot chamfer: no part")
            dist = val("distance")
            with BuildPart() as p:
                add(ctx.part)
