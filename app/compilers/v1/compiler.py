...
class FeatureGraphCompilerV1:
    ...
    def compile(self, graph: FeatureGraphV1):
        ...
        try:
            ...
            trace.append(
                TraceEvent(stage="sketch_compile", target=None, status="success")
            )

            # 2. Compile features (3D) using the sketches
            solid = self.feature_compiler.compile(graph, sketches)
            trace.append(
                TraceEvent(stage="feature_compile", target=None, status="success")
            )
            ...
            if has_critical_geometry_issues(geometry_issues):
                ...
                return solid, ExecutionTrace(success=False, events=trace)

            # Validation passed
            trace.append(
                TraceEvent(stage="geometry_validation", target=None, status="success")
            )

            return solid, ExecutionTrace(success=True, events=trace)

        except Exception as e:
            trace.append(
                TraceEvent(
                    stage="compile", target=None, status="failure", message=str(e)
                )
            )
            return None, ExecutionTrace(success=False, events=trace)
