...
from app.domain.feature_graph_v2 import FeatureGraphV2, FeatureV2, SemanticSelector
...
class Build123dCompilerV3(Build123dCompilerV2):
    ...
    def __init__(self, output_dir: Path = Path("outputs")):
        ...
        logger.info(
            f"Build123dCompilerV3 initialized with {len(self.validators)} validators"
        )

    def compile(
        self, feature_graph: FeatureGraphV2, job_id: str
        ...
        try:
            ...
            for sketch_def in feature_graph.sketches:
                events.append(
                    TraceEvent(
                        stage="sketch_compile", target=sketch_def.id, status="pending"
                    )
                )
            ...
            for feature in feature_graph.features:
                ...
                try:
                    ...
                    events[-1].status = "success"
                    events[-1].message = (
                        f"Created {len(ctx.registry.feature_map.get(feature.id, []))} entities"
                    )
            ...
            if hasattr(e, "compiler_error") and e.compiler_error:
                ...
                logger.info(
                    f"Structured error available: {e.compiler_error.error_type}"
                )

            trace = ExecutionTrace(
                success=False, events=events, retryable=True, metadata=metadata
            )
            raise
        except Exception as e:
            logger.error(f"V3 Compilation failed: {e}")
    ...
    def _apply_feature_v3(
        ...
        if feature.type == "extrude":
            ...
            logger.warning(
                f"Feature type '{feature.type}' not yet V3-enabled, using V2"
            )
            solid = self._apply_feature_v2(
                feature, ctx.sketches, ctx.graph, current_solid
            )
    ...
    def _validate_geometry(
        self, solid: Solid, feature: FeatureV2, ctx: CompilationContext
    ) -> None:
        """
            ...
            try:
                ...
                if error:
                    ...
                    raise FeatureCompilationError(
                        error.to_trace_message(), compiler_error=error
                    )
            except FeatureCompilationError:
                raise
            except Exception as e:
    ...
    def _apply_extrude_v3(
        ...
        depth = self._resolve_param(
            feature.params.get("depth") or feature.params.get("distance"), ctx.graph
        )

        # Create geometry
        sketch_builder = ctx.sketches[sketch_id]
        target_geo = (
            sketch_builder.sketch
            if hasattr(sketch_builder, "sketch")
            else sketch_builder
        )
    ...
    def _apply_fillet_v3(
        ...
        edges = self._resolve_semantic_selector_v3(
            feature.topology_refs, current_solid, ctx
        )

        if not edges:
            logger.warning(
                f"No edges selected for fillet '{feature.id}', using all edges"
            )
            edges = list(current_solid.edges())

        logger.info(
            f"Fillet '{feature.id}' targeting {len(edges)} edges (V3 selection)"
        )
    ...
    def _tag_solid_entities(
        ...
        for edge in solid.edges():
            ...
            identity = EntityIdentity(
                created_by=feature.id, role=role, axis=axis, feature_type=feature.type
            )
    ...
    def _find_edge_id(self, edge: Edge, ctx: CompilationContext) -> Optional[str]:
        ...
        for eid, existing_edge in ctx.edge_map.items():
            try:
                if (
                    abs(existing_edge.center().X - center.X) < 0.001
                    and abs(existing_edge.center().Y - center.Y) < 0.001
                    and abs(existing_edge.center().Z - center.Z) < 0.001
                    and abs(existing_edge.length - length) < 0.001
                ):
                    return eid
            except Exception:
                continue
    ...
    def _resolve_semantic_selector_v3(
        ...
        if selector.entity_ids:
            logger.info(
                f"V3: Using explicit entity IDs (Tier 1): {selector.entity_ids}"
            )
            edges = []
        ...
        if selector.created_by_feature:
            logger.info(
                f"V3: Selecting by feature origin (Tier 2): {selector.created_by_feature}"
            )
            identities = ctx.registry.get_by_feature(selector.created_by_feature)
            edges = [
                ctx.edge_map[identity.id]
        ...
        if selector.semantic_roles:
            logger.info(
                f"V3: Selecting by semantic roles (Tier 3): {selector.semantic_roles}"
            )
            all_edges = []
        ...
        # TIER 4: Geometric fallback (V2 logic)
        logger.warning(
            "V3: Falling back to geometric selection (Tier 4) - non-deterministic!"
        )
