...
class Build123dCompilerV2:
            ...
            for feature in feature_graph.features:
                ...
                try:
                    solid = self._apply_feature_v2(
                        feature, sketches, feature_graph, solid
                    )
                    events[-1].status = "success"
                except Exception as e:
                    events[-1].status = "failure"
    ...
    def _compile_sketch(self, sketch_def: SketchGraphV2, graph: FeatureGraphV2) -> Any:
    ...
    def _apply_feature_v2(
        ...
        if feature.type == "extrude":
            ...
            depth = self._resolve_param(
                feature.params.get("depth") or feature.params.get("distance"), graph
            )
            ...
            edges = self._get_edges_from_topology_refs(
                feature.topology_refs, current_solid, graph
            )

            if edges:
                return fillet(
                    current_solid.edges().filter_by(lambda e: e in edges), radius=radius
                )
            else:
            ...
            if edges:
                return chamfer(
                    current_solid.edges().filter_by(lambda e: e in edges),
                    length=distance,
                )
            else:
    ...
    def _resolve_semantic_selector(
        ...
        elif selector.selector_type in [
            SelectorType.SEMANTIC,
            SelectorType.FILTER_CHAIN,
        ]:
    ...
    def _apply_filter(
        self, edges: List[Edge], filter_def: GeometricFilter, solid: Solid
    ) -> List[Edge]:
