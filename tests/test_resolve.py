...
IMPOSSIBLE = (
    "Support a rotating shaft carrying 1.2 kN at 600 rpm and "
...
def test_the_dependency_walk_terminates():
    assert len(RS.dependents("shaft_dia_mm")) == len(set(RS.dependents("shaft_dia_mm")))
...
def test_the_resolved_design_still_carries_the_stated_duty():
    ...
    bearing = next(
        r for r in result.chosen.plan.resolutions if r.function == F.SUPPORTS_ROTATION
    )
...
def test_the_chosen_design_is_the_smallest_that_works():
    ...
    assert all(a.metrics["outside_dia_mm"] >= chosen for a in result.alternatives)
...
def test_unsatisfiable_names_the_smallest_change_that_would_help():
    ...
    assert any(word in result.smallest_change for word in ("larger", "spline", "keys"))
...
def test_never_only_a_conflict():
    ...
    for request in (JOINT, SIZED, IMPOSSIBLE):
        ...
        assert result.outcome in (RS.RESOLVED, RS.ALTERNATIVES, RS.UNSATISFIABLE)
