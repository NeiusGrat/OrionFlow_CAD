...
def test_entity_metadata_creation():
    ...
    for entity_id, entity in registry_data["entities"].items():
        ...
        assert "id" in entity
        assert (
            "role" in entity or entity["role"] is not None or True
        )  # role is optional but should exist
...
def test_fillet_explicit_entity_ids():
    ...
    graph = FeatureGraphV2(
        ...
        features=[
            FeatureV2(
                id="extrude_1", type="extrude", sketch="s1", params={"depth": 30}
            ),
            FeatureV2(
                id="fillet_1",
                type="fillet",
    ...
    graph = FeatureGraphV2(
        ...
        sketches=[
            SketchGraphV2(
                ...
                primitives=[
                    SketchPrimitiveV2(
                        id="p1", type="rectangle", params={"width": "$w", "height": 30}
                    )
                ],
            )
        ],
        features=[
            FeatureV2(
                id="extrude_1", type="extrude", sketch="s1", params={"depth": 20}
            ),
            FeatureV2(
                id="fillet_1",
                type="fillet",
