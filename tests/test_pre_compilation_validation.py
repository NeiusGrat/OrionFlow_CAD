...
class TestSanityRules:
    ...
    def test_positive_depth_valid(self):
        ...
        ir = MockIR(features=[MockFeature("f1", "extrude", {"depth": 10.0})])
    ...
    def test_positive_depth_zero_fails(self):
        ...
        ir = MockIR(features=[MockFeature("f1", "extrude", {"depth": 0})])
        rule = PositiveDepthRule()
        issues = rule.validate(ir, ManufacturingConstraints())
        assert len(issues) == 1
    ...
    def test_positive_depth_negative_fails(self):
        ...
        ir = MockIR(features=[MockFeature("f1", "extrude", {"depth": -5.0})])
        rule = PositiveDepthRule()
        issues = rule.validate(ir, ManufacturingConstraints())
        assert len(issues) == 1
    ...
    def test_positive_radius_valid(self):
        ...
        ir = MockIR(
            sketches=[MockSketch("s1", [MockPrimitive("circle", {"radius": 10.0})])]
        )
    ...
    def test_positive_radius_zero_fails(self):
        ...
        ir = MockIR(
            sketches=[MockSketch("s1", [MockPrimitive("circle", {"radius": 0})])]
        )
        rule = PositiveRadiusRule()
        issues = rule.validate(ir, ManufacturingConstraints())
        assert len(issues) == 1
    ...
    def test_positive_fillet_radius_valid(self):
        ...
        ir = MockIR(features=[MockFeature("f1", "fillet", {"radius": 2.0})])
        rule = PositiveRadiusRule()
        issues = rule.validate(ir, ManufacturingConstraints())
        assert len(issues) == 0
    ...
    def test_positive_dimension_valid(self):
        ...
        ir = MockIR(
            sketches=[
                MockSketch(
                    "s1", [MockPrimitive("rectangle", {"width": 50.0, "height": 30.0})]
                )
            ]
        )
    ...
    def test_positive_dimension_zero_width_fails(self):
        ...
        ir = MockIR(
            sketches=[
                MockSketch(
                    "s1", [MockPrimitive("rectangle", {"width": 0, "height": 30.0})]
                )
            ]
        )
        rule = PositiveDimensionRule()
...
class TestPhysicsRules:
    ...
    def test_fillet_radius_too_small_warns(self):
        ...
        ir = MockIR(features=[MockFeature("f1", "fillet", {"radius": 0.5})])
        rule = FilletRadiusRule()
...
class TestManufacturingRules:
    ...
    def test_wall_thickness_too_thin_warns(self):
        ...
        ir = MockIR(features=[MockFeature("f1", "extrude", {"depth": 0.5})])
        rule = MinWallThicknessRule()
        issues = rule.validate(ir, constraints)
        assert len(issues) == 1
        assert issues[0].severity == ValidationSeverity.WARNING
...
class TestPreCompilationValidator:
    ...
    def test_valid_ir_passes(self):
        ...
        ir = MockIR(
            ...
            features=[MockFeature("f1", "extrude", {"depth": 10.0})],
            parameters={},
        )
    ...
    def test_invalid_ir_fails(self):
        ...
        ir = MockIR(
            sketches=[],
            features=[MockFeature("f1", "extrude", {"depth": -5.0})],  # Invalid
            parameters={},
        )
        validator = PreCompilationValidator()
        result = validator.validate(ir)
    ...
    def test_validate_or_raise(self):
        ...
        ir = MockIR(
            sketches=[],
            features=[MockFeature("f1", "extrude", {"depth": 0})],  # Invalid
            parameters={},
        )
        validator = PreCompilationValidator()
...
class TestConvenienceFunctions:
    ...
    def test_validate_for_3d_printing(self):
        ...
        ir = MockIR(
            sketches=[],
            features=[MockFeature("f1", "extrude", {"depth": 0.3})],  # Very thin
            parameters={},
        )
