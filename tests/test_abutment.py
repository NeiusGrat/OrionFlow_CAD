...
def _row(bore=25.0, r=1.0, da_min=30.0, Da_max=47.0, ra_max=1.0, page=1):
    return A.AbutmentRow(bore=bore, r_min=r, da_min=da_min, Da_max=Da_max,
                         ra_max=ra_max, page=page)
...
def test_the_stand_off_is_equal_on_both_sides():
    ...
    assert check(_bearing(d=90.0, D=140.0),
                 _row(bore=90.0, r=1.0, da_min=95.0, Da_max=135.0)) is None
    # The same row against a 150 outside diameter stands off 15 on one side.
    fail = check(_bearing(d=90.0, D=150.0),
...
def test_a_unique_physically_valid_row_is_attributed():
    ...
    rows = [_row(bore=90.0, r=1.0, da_min=95.0, Da_max=135.0),
            _row(bore=90.0, r=2.0, da_min=101.0, Da_max=149.0)]   # a 160 OD
...
def test_two_valid_rows_are_ambiguous_and_neither_is_taken():
    ...
    rows = [_row(bore=25.0, r=0.3, da_min=27.0, Da_max=45.0, ra_max=0.3),
...
def test_a_bearing_from_another_bore_group_is_never_matched():
    match = A.attribute(_bearing(d=30.0, D=62.0),
                        [_row(bore=25.0, r=1.0, da_min=31.0, Da_max=57.0)])
    assert match.verdict == A.REJECTED
...
def test_the_bore_heading_is_gated_by_the_verified_catalogue():
    ...
    assert rows and all(r.bore == 45.0 for r in rows), \
...
def test_confidence_is_attributed_and_never_climbs():
    ...
    bearing = _bearing(d=90.0, D=140.0, designation="6018")
    match = A.attribute(bearing, [_row(bore=90.0, r=1.0, da_min=95.0,
                                       Da_max=135.0)])
...
def test_every_field_the_contract_requires_is_present():
    ...
    match = A.attribute(bearing, [_row(bore=90.0, r=1.0, da_min=95.0,
                                       Da_max=135.0)])
    record = A.provenance(bearing, match, object())
    for field in ("designation", "bearing_family", "bore", "outside_diameter",
                  "width", "da_min", "Da_max", "r_min", "matching_constraints",
                  "confidence", "source_document", "source_pages"):
...
class TestHarvested:
    ...
    def test_every_attributed_row_satisfies_every_constraint(self):
        ...
        for designation, row in self.data()["abutments"].items():
            bearing = {"designation": designation, "d": row["bore"],
                       "D": row["outside_diameter"], "B": row["width"]}
            candidate = A.AbutmentRow(
                bore=row["bore"], r_min=row["r_min"], da_min=row["da_min"],
                Da_max=row["Da_max"], ra_max=row["ra_max"])
            for name, check in A.CONSTRAINTS.items():
                assert check(bearing, candidate) is None, \
    ...
    def test_the_shoulder_is_geometrically_possible(self):
        ...
        shoulder."""
        for designation, row in self.data()["abutments"].items():
            assert row["bore"] < row["da_min"] < row["Da_max"] \
                < row["outside_diameter"], designation

    def test_unresolved_rows_are_reported_not_dropped(self):
        data = self.data()
        assert data["unresolved"], "ambiguity and rejection are outcomes"
        assert data["gate"]["ambiguous"] + data["gate"]["rejected"] \
            == len(data["unresolved"])
