...
from app.compilers.kernel_adapter import KernelRegistry, Build123dAdapter
from app.domain.versioning import VersionManager
...
def test_version_manager_commit():
    ...
    graph_v1 = FeatureGraphV3(
        version="3.0", parameters={"width": 100.0}, sketches=[], features=[]
    )
...
def test_version_manager_undo():
    ...
    graph_v2 = FeatureGraphV3(
        version="3.0", parameters={"width": 200.0}, sketches=[], features=[]
    )

    vm = VersionManager()
    v1 = vm.commit(graph_v1, "Version 1")
    vm.commit(graph_v2, "Version 2 - increased width")
...
def test_version_history():
    ...
    for i in range(3):
        graph = FeatureGraphV3(
            version="3.0", parameters={"iteration": i}, sketches=[], features=[]
        )
