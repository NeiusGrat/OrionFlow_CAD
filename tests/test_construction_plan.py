...
class TestConstructionPlan:
    ...
    def test_full_plan(self):
        ...
        plan = ConstructionPlan(
            ...
            construction_sequence=[
                ConstructionStep(
                    order=1,
                    description="Create base sketch: rectangle",
                    feature_type="sketch",
                ),
                ConstructionStep(
                    order=2, description="Extrude symmetrically", feature_type="extrude"
                ),
                ConstructionStep(
                    order=3,
                    description="Apply fillet only on top edges",
                    feature_type="fillet",
                ),
            ],
            parameters={
                ...
                "fillet_radius": PlanParameter(
                    unit="mm", default=5, depends_on="height"
                ),
            },
    ...
    def test_circular_dependency_validation(self):
        ...
        with pytest.raises(ValidationError, match="circular dependency"):
            ConstructionPlan(
                base_reference="XY plane",
                construction_sequence=[
                    ConstructionStep(order=1, description="Create sketch")
                ],
                parameters={
                    "width": PlanParameter(
                        default=50, depends_on="width"
                    )  # Self-reference
                },
            )
    ...
    def test_invalid_range_validation(self):
        ...
        plan = ConstructionPlan(
            ...
            parameters={
                "width": PlanParameter(
                    default=50, min_value=100, max_value=10
                )  # min > max
            },
        )
    ...
    def test_default_below_min_validation(self):
        ...
        plan = ConstructionPlan(
            ...
            parameters={"width": PlanParameter(default=5, min_value=10, max_value=100)},
        )
        errors = plan.validate_plan()
    ...
    def test_to_prompt_context(self):
        ...
        plan = ConstructionPlan(
            ...
            parameters={"width": PlanParameter(unit="mm", default=50)},
    ...
    def test_plan_to_feature_graph_compatibility(self):
        ...
        plan = ConstructionPlan(
            ...
            construction_sequence=[
                ConstructionStep(
                    order=1,
                    description="Create base sketch: rectangle with width and height parameters",
                    feature_type="sketch",
                ),
                ConstructionStep(
                    order=2, description="Extrude to depth", feature_type="extrude"
                ),
            ],
            parameters={
